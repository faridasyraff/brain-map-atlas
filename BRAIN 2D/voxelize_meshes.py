"""
voxelize_meshes.py — convert Allen CCFv3 .obj meshes into a 3D voxel volume.
...
def voxelize_one_mesh(mesh_path: Path, grid_origin_um, grid_shape_vox, voxel_um):
    ...
    Allen .obj mesh axis convention (verified empirically):
...
def main():
    ...
    shell_path = meshes_dir / f"{args.shell_id}.obj"
    if not shell_path.exists():
        ...
        shell = trimesh.load(str(shell_path), process=False)
    ...
    # Collect mesh files
    mesh_files = sorted(p for p in meshes_dir.glob("*.obj")
                        if p.stat().st_size > 0)
    if args.limit > 0:
    ...
    #   because the .obj vertex convention is (L-R, S-I, A-P)
