...
function TransverseView() {
  ...
  const loadSliceImages = (sliceIndex) => {
    ...
    brainImg.src = `/slices/transverse/brain_${String(sliceIndex).padStart(4, '0')}.png`;
    labelImg.src = `/slices/transverse/labels_${String(sliceIndex).padStart(4, '0')}.png`;

    brainImg.onload = () => {
      const canvas = canvasRef.current;
      if (canvas) {
        canvas.width = brainImg.height;
        canvas.height = brainImg.width;
        redraw();
      }
    };
